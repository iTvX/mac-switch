...
final class PackageSmokeTests: XCTestCase {
    ...
    func testDashboardUsesAdaptiveNativePopoverLayout() throws {
        ...
        XCTAssertTrue(controlRowSource.contains(".contextMenu"))
        XCTAssertTrue(controlRowSource.contains("Label(\"Settings...\", systemImage: \"slider.horizontal.3\")"))
        XCTAssertTrue(controlRowSource.contains("store.preferredCustomizeKind = kind"))
        XCTAssertTrue(controlRowSource.contains("store.preferredPreferencesTab = \"customize\""))
        XCTAssertTrue(controlRowSource.contains("NotificationCenter.default.post(name: .openMacSwitchPreferences, object: nil)"))
        XCTAssertTrue(controlRowSource.contains("Label(\"Remove from Menu\", systemImage: \"eye.slash\")"))
        XCTAssertTrue(controlRowSource.contains("store.setEnabled(kind, false)"))
        XCTAssertTrue(controlRowSource.contains(".disabled(store.isCustomizationBusy(kind) || store.visibleKinds.count <= 1)"))
