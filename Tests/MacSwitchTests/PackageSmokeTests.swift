...
final class PackageSmokeTests: XCTestCase {
    ...
    func testReleaseScriptKeepsSigningNotaryAndArchiveGuards() throws {
        ...
        XCTAssertTrue(releaseWorkflow.contains("git tag \"$tag\" \"$GITHUB_SHA\""), "automatic release should tag the merged main commit")
        XCTAssertTrue(releaseWorkflow.contains("git push origin \"refs/tags/$tag\""), "automatic release should publish the generated tag")
        XCTAssertTrue(releaseWorkflow.contains("Release tag $tag must point to a commit reachable from $GITHUB_DEFAULT_BRANCH"))
