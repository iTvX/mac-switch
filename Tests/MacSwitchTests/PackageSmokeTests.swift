...
final class PackageSmokeTests: XCTestCase {
    func testSafeSelfTestExecutablePasses() throws {
        ...
        XCTAssertTrue(result.output.contains("Night Shift enables the current effect instead of toggling the master state"), result.combinedOutput)
        XCTAssertTrue(result.output.contains("Night Shift repairs legacy inactive state before enabling"), result.combinedOutput)
        XCTAssertTrue(result.output.contains("Night Shift disables the effect without disabling future schedules"), result.combinedOutput)
    ...
    func testDashboardUsesAdaptiveNativePopoverLayout() throws {
        ...
        XCTAssertTrue(reorderRowSource.contains("let isPlaceholder: Bool"))
        XCTAssertTrue(reorderRowSource.contains(".opacity(isPlaceholder ? 0 : 1)"))
        XCTAssertTrue(reorderRowSource.contains("reportsFrame: true"))
        ...
        XCTAssertTrue(controlRowSource.contains(".offset(x: isOn ? 22 : 2)"))
        XCTAssertFalse(controlRowSource.contains("Toggle(\"\", isOn: Binding"))
        ...
        XCTAssertTrue(dashboardSource.contains("@State private var dashboardQuickMenuOpeningEventNumber: Int?"))
        XCTAssertTrue(dashboardSource.contains("@State private var dashboardRowFrames: [SwitchKind: CGRect] = [:]"))
        XCTAssertTrue(dashboardSource.contains("quickMenuKind: $dashboardQuickMenuKind"))
        XCTAssertTrue(dashboardSource.contains("quickMenuOpeningEventNumber: $dashboardQuickMenuOpeningEventNumber"))
        XCTAssertTrue(dashboardSource.contains("DashboardRowQuickMenu("))
        XCTAssertTrue(dashboardSource.contains(".position(quickMenuPosition(for: rowFrame, kind: kind))"))
        ...
        XCTAssertTrue(dashboardSource.contains("private func quickMenuPosition(for rowFrame: CGRect, kind: SwitchKind)"))
        ...
        XCTAssertTrue(dashboardSource.contains("DashboardQuickMenuDismissLayer {"))
        XCTAssertTrue(dashboardSource.contains(".ignoredEventNumber(dashboardQuickMenuOpeningEventNumber)"))
        XCTAssertTrue(dashboardSource.contains(".rightClickPassthrough(rowFrames: dashboardRowFrames, panelHeight: panelSize.height)"))
        XCTAssertTrue(dashboardSource.contains(".frame(width: panelSize.width, height: panelSize.height)"))
        ...
        XCTAssertTrue(controlRowSource.contains("@Binding var quickMenuOpeningEventNumber: Int?"))
        XCTAssertTrue(controlRowSource.contains("GeometryReader { proxy in"))
        XCTAssertTrue(controlRowSource.contains("DashboardRowFramePreferenceKey.self"))
        ...
        XCTAssertTrue(controlRowSource.contains("var rightClickPassthroughRowFrames: [SwitchKind: CGRect] = [:]"))
        XCTAssertTrue(controlRowSource.contains("func rightClickPassthrough(rowFrames: [SwitchKind: CGRect], panelHeight: CGFloat) -> Self"))
        XCTAssertTrue(controlRowSource.contains("shouldPassRightClickThrough(at: point, event: window?.currentEvent ?? NSApp.currentEvent)"))
        XCTAssertTrue(controlRowSource.contains("let swiftUIPoint = CGPoint(x: point.x, y: rightClickPassthroughPanelHeight - point.y)"))
        XCTAssertTrue(controlRowSource.contains("rightClickPassthroughRowFrames.values.contains { $0.contains(swiftUIPoint) }"))
        XCTAssertTrue(controlRowSource.contains("private func isRightClickEvent(_ event: NSEvent?) -> Bool"))
        XCTAssertTrue(controlRowSource.contains("func ignoredEventNumber(_ eventNumber: Int?) -> Self"))
        XCTAssertTrue(controlRowSource.contains("guard event.eventNumber != ignoredEventNumber else { return }"))
        XCTAssertTrue(controlRowSource.contains("override func hitTest(_ point: NSPoint) -> NSView?"))
        XCTAssertTrue(controlRowSource.contains("override func mouseDown(with event: NSEvent)"))
        XCTAssertTrue(controlRowSource.contains("override func rightMouseDown(with event: NSEvent)"))
        XCTAssertTrue(controlRowSource.contains("override func scrollWheel(with event: NSEvent)"))
        XCTAssertTrue(controlRowSource.contains("private struct DashboardRightClickDetector"))
        XCTAssertTrue(controlRowSource.contains("private final class DashboardRightClickDetectorView"))
        XCTAssertTrue(controlRowSource.contains("DashboardRightClickDetector {"))
        XCTAssertTrue(controlRowSource.contains("rightMouseDown(with event: NSEvent)"))
        XCTAssertTrue(controlRowSource.contains("event.type == .rightMouseDown"))
        XCTAssertTrue(controlRowSource.contains("event.type == .leftMouseDown && event.modifierFlags.contains(.control)"))
        XCTAssertTrue(controlRowSource.contains("title: \"Configure\""))
        XCTAssertTrue(controlRowSource.contains("symbol: \"slider.horizontal.3\""))
        XCTAssertTrue(dashboardSource.contains("store.preferredCustomizeKind = kind"))
        XCTAssertTrue(dashboardSource.contains("store.preferredPreferencesTab = \"customize\""))
        XCTAssertTrue(dashboardSource.contains("NotificationCenter.default.post(name: .openMacSwitchPreferences, object: nil)"))
        XCTAssertTrue(controlRowSource.contains("title: \"Hide from Menu\""))
        XCTAssertTrue(controlRowSource.contains("symbol: \"eye.slash\""))
        XCTAssertTrue(dashboardSource.contains("Keep at least one item"))
        XCTAssertTrue(dashboardSource.contains("Finish current action first"))
        XCTAssertTrue(dashboardSource.contains("store.setEnabled(kind, false)"))
        XCTAssertTrue(controlRowSource.contains("hideDisabledReason != nil"))
        ...
        XCTAssertTrue(appDelegate.contains("configureDashboardRoundedMask(on: hostingView)"))
        XCTAssertTrue(appDelegate.contains("configureDashboardRoundedMask(on: window.contentView)"))
        XCTAssertTrue(appDelegate.contains("configureDashboardRoundedMask(on: window.contentView?.superview)"))
        XCTAssertTrue(appDelegate.contains("view.layer?.backgroundColor = NSColor.clear.cgColor"))
        XCTAssertTrue(appDelegate.contains("view.layer?.isOpaque = false"))
        XCTAssertTrue(appDelegate.contains("view.layer?.cornerRadius = DashboardLayout.cornerRadius"))
        XCTAssertTrue(appDelegate.contains("view.layer?.cornerCurve = .continuous"))
        XCTAssertTrue(appDelegate.contains("view.layer?.masksToBounds = true"))
    ...
    func testPreferencesWindowIsVerticallyResizableAndScreenClamped() throws {
        ...
        XCTAssertTrue(appDelegate.contains("private var preferencesResizeTimer: Timer?"))
        XCTAssertTrue(appDelegate.contains("private func preferencesMaximumContentHeight(for window: NSWindow) -> CGFloat"))
        XCTAssertTrue(appDelegate.contains("func windowWillResize(_ sender: NSWindow, to frameSize: NSSize) -> NSSize"))
        XCTAssertTrue(appDelegate.contains("guard sender === preferencesWindow else { return frameSize }"))
        XCTAssertTrue(appDelegate.contains("return constrainedPreferencesFrameSize(for: sender, proposedFrameSize: frameSize)"))
        ...
        XCTAssertTrue(appDelegate.contains("matching: [.leftMouseDown, .rightMouseDown, .otherMouseDown, .keyDown]"))
        XCTAssertTrue(appDelegate.contains("event.type == .leftMouseDown || event.type == .rightMouseDown || event.type == .otherMouseDown"))
        XCTAssertTrue(appDelegate.contains("matching: [.leftMouseDown, .rightMouseDown, .otherMouseDown]"))
        ...
        XCTAssertTrue(appDelegate.contains("let currentContentHeight = window.contentView?.bounds.height ?? baseContentSize.height"))
        ...
        XCTAssertTrue(appDelegate.contains("window.isMovableByWindowBackground = preferencesWindowWasMovableByBackground"))
        XCTAssertTrue(appDelegate.contains("Timer(timeInterval: 1.0 / 60.0, repeats: true)"))
        XCTAssertTrue(appDelegate.contains("timer.tolerance = 1.0 / 240.0"))
        XCTAssertTrue(appDelegate.contains("RunLoop.main.add(timer, forMode: .eventTracking)"))
        XCTAssertTrue(appDelegate.contains("preferencesResizeTimer?.invalidate()"))
        XCTAssertTrue(appDelegate.contains("window.setFrame(frame, display: display)"))
        ...
        XCTAssertFalse(appDelegate.contains("window.minSize = NSSize(width: 540, height: 390)"))
    ...
    func testReleaseScriptKeepsSigningNotaryAndArchiveGuards() throws {
        ...
        XCTAssertTrue(prCheckWorkflow.contains("/Applications/Xcode_16.2.app"), "macOS 14 runners should select an installed Swift 6 toolchain")
        XCTAssertTrue(prCheckWorkflow.contains("Swift\\ version\\ 6\\."), "compatibility jobs should reject an obsolete default Swift toolchain")
        ...
        XCTAssertTrue(releaseWorkflow.contains("\".github/workflows/**\""), "workflow-only changes should not trigger notarized app releases")
        XCTAssertTrue(releaseWorkflow.contains("\"Tests/**\""), "test-only changes should not trigger notarized app releases")
        ...
        XCTAssertTrue(releaseWorkflow.contains("update_channel:"), "manual releases should choose a Sparkle update channel")
        XCTAssertTrue(releaseWorkflow.contains("build_number:"), "manual releases should support paired stable/beta build numbers")
        ...
        XCTAssertTrue(releaseWorkflow.contains("echo \"build_number=$build_number\" >> \"$GITHUB_OUTPUT\""), "release job should pass paired build numbers to the build script")
        ...
        XCTAssertTrue(script.contains("notary_profile_history"), "release script should capture notarytool validation output")
        XCTAssertTrue(script.contains("Apple rejected notarization credential validation"), "release script should explain Apple agreement failures")
        XCTAssertTrue(script.contains("required Apple Developer agreement is missing or expired"), "release script should identify expired or missing Apple agreements")
        ...
        XCTAssertTrue(readinessScript.contains("notary_profile_history"), "release readiness should capture notarytool validation output")
        XCTAssertTrue(readinessScript.contains("Apple Developer agreement is missing or expired"), "release readiness should explain Apple agreement failures")
    ...
    func testSparkleUpdateFlowIsBundledSignedAndPublished() throws {
        ...
        XCTAssertTrue(appcastScript.contains("APPCAST_EXISTING_URL"))
        ...
        XCTAssertTrue(publishScript.contains("APPCAST_EXPECTED_RELEASE_TAG"))
    ...
    func testKeepAwakeIntentRestoresAfterUpdateRelaunch() throws {
        ...
        XCTAssertTrue(views.contains("store.setKeepAwakeDuration(duration)"))
    ...
    func testDashboardUsesCustomGestureReorderingWithoutSystemDragImages() throws {
        ...
        XCTAssertTrue(dashboardSource.contains("@State private var dashboardDragState: DashboardDragState?"))
        XCTAssertTrue(views.contains("private struct DashboardFloatingDragRow: View"))
        XCTAssertTrue(views.contains("reportsFrame: false"))
    ...
    func testPreferencePanelsDoNotSynchronouslyPollPermissionsMediaOrNightShift() throws {
        ...
        XCTAssertTrue(nightShiftSource.contains("@State private var scheduleMode: NightShiftScheduleMode = .off"))
        XCTAssertTrue(nightShiftSource.contains("@State private var customSchedule = NightShiftScheduleState.defaultSchedule"))
        ...
        XCTAssertTrue(nightShiftSource.contains("updateNightShiftScheduleMode(value)"))
        XCTAssertTrue(nightShiftSource.contains("applyNightShiftCustomSchedule()"))
        ...
        XCTAssertTrue(nightShiftSource.contains("let latest = NightShiftPreferences.state"))
        XCTAssertTrue(nightShiftSource.contains("NightShiftScheduleMode.allCases"))
        XCTAssertTrue(nightShiftSource.contains("External-display results vary by hardware."))
        XCTAssertTrue(nightShiftSource.contains("publisher(for: .nightShiftStatusDidChange)"))
        XCTAssertTrue(nightShiftSource.contains("guard !isRefreshingNightShift, !isUpdatingNightShiftSchedule else"))
        XCTAssertFalse(nightShiftSource.contains("autoScheduleEnabled"))
        XCTAssertFalse(nightShiftSource.contains("setAutoScheduleEnabled"))
    ...
    func testStatefulSwitchesReadBackSystemStateAfterMutatingCalls() throws {
        ...
        let nightShiftSource = try extract(
            systemSwitches,
            from: "final class NightShiftSwitch",
        ...
        XCTAssertTrue(nightShiftSource.contains("NightShiftStatePolicy.mutations(toReach: enabled, from: initial)"))
        XCTAssertTrue(nightShiftSource.contains("client.setEnabled(target)"))
        XCTAssertTrue(nightShiftSource.contains("state.enabled"))
        XCTAssertTrue(nightShiftSource.contains("state.active"))
        XCTAssertFalse(nightShiftSource.contains("client.setActive(false)"))
        XCTAssertTrue(nightShiftSource.contains("waitForState({ NightShiftStatePolicy.reached(enabled, state: $0) })"))
        ...
        XCTAssertTrue(systemSwitches.contains("private final class NightShiftCoreBrightnessClient"))
        XCTAssertTrue(systemSwitches.contains("static let shared = NightShiftCoreBrightnessClient()"))
        XCTAssertTrue(systemSwitches.contains("private let statusBufferSize = 64"))
        XCTAssertTrue(systemSwitches.contains("setStatusNotificationBlock:"))
        XCTAssertTrue(systemSwitches.contains("enableNotifications"))
        XCTAssertTrue(systemSwitches.contains("statusNotificationPending"))
    ...
    func testMomentaryActionsExposeInProgressFeedback() throws {
        ...
        XCTAssertTrue(keepAwakeDurationMenuSource.contains("store.setKeepAwakeWhenLidClosed(value)"))
        XCTAssertTrue(keepAwakeDurationMenuSource.contains("ForEach(KeepAwakeDuration.allCases)"))
    ...
    func testInteractionPolishAndStaleActionGuards() throws {
        ...
        XCTAssertTrue(model.contains("let target = !decorated.isOn"))
