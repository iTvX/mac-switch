...
final class PackageSmokeTests: XCTestCase {
    ...
    func testDashboardUsesAdaptiveNativePopoverLayout() throws {
        ...
        XCTAssertTrue(controlRowSource.contains("@State private var visualIsOn = false"))
        XCTAssertTrue(controlRowSource.contains("@State private var pendingVisualTarget: Bool?"))
        XCTAssertTrue(controlRowSource.contains("private static let thumbAnimation = Animation.spring(response: 0.30"))
        XCTAssertTrue(controlRowSource.contains("private func performOptimisticToggle()"))
        XCTAssertTrue(controlRowSource.contains("scheduleFallbackReconcile(for: target, interactionID: interactionID)"))
        XCTAssertTrue(controlRowSource.contains(".frame(width: 44, height: 24)"))
        XCTAssertTrue(controlRowSource.contains(".offset(x: visualIsOn ? 22 : 2)"))
        XCTAssertTrue(controlRowSource.contains("return isBusy ? 0.92 : 1"))
