...
final class PackageSmokeTests: XCTestCase {
    ...
    func testSwitchModesAreBuiltInRestorableAndSafeByDefault() throws {
        ...
        XCTAssertTrue(storeSource.contains("Switch the update channel to Beta before starting a mode."))
        XCTAssertTrue(storeSource.contains("Turn this mode off before hiding it from the menu."))
        ...
        XCTAssertTrue(appDelegate.contains("visibleModeCount: store.visibleModes.count"))
        XCTAssertTrue(appDelegate.contains("!store.activeModeSessions.isEmpty, softwareUpdates.updateChannel != .beta"))
    }
    ...
    func testPreferencesWindowIsVerticallyResizableAndScreenClamped() throws {
        ...
        XCTAssertTrue(appDelegate.contains("addCursorRect(bounds, cursor: .resizeUpDown)"))
    ...
    func testSparkleUpdateFlowIsBundledSignedAndPublished() throws {
        ...
        XCTAssertTrue(manager.contains("var requiresBetaChannel: (() -> Bool)?"))
        XCTAssertTrue(manager.contains("if requiresBetaChannel?() == true"))
        XCTAssertTrue(manager.contains("bestItem(in: appcast.items, for: .beta)"))
        ...
        XCTAssertTrue(manager.contains("canInstall(item) else"), "automatic mode recovery should never install an older beta")
        ...
        XCTAssertTrue(appDelegate.contains("softwareUpdates.requiresBetaChannel"))
        ...
        XCTAssertTrue(aboutPreferencesSource.contains("Picker(\"\", selection: updateChannelBinding)"))
        XCTAssertTrue(aboutPreferencesSource.contains("Turn off the active mode before changing update channels."))
